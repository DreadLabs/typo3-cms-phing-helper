<?php
class Seclib_SftpTask extends Task {
	...
	/**
	 * Sets the public key file of the user to sftp
	 */
	...
	/**
	 * Sets the private key file of the user to sftp
	 */
	...
	/**
	 * Sets the private key file passphrase of the user to sftp
	 */
	...
	public function main() {
		...
		if (FALSE === class_exists('Net_SSH2')) {
			throw new BuildException("To use SftTask, you need to install the package phpseclib/phpscelib.");
		}
...
// 		$this->connection->disconnect();
	...
	protected function copyFile($local, $remote) {
		...
		if ($this->fetch) {
			...
			$ret = $this->sftp->get($remoteEndpoint, $localEndpoint);
			...
			if (!is_null($this->mode)) {
				...
				$ret = $this->sftp->put($remoteEndpoint, $localEndpoint, NET_SFTP_LOCAL_FILE);
			} else {
				$ret = $this->sftp->put($remoteEndpoint, $localEndpoint, NET_SFTP_LOCAL_FILE);
			}
